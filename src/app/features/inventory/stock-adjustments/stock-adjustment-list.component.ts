...
import { STOCK_ADJUSTMENT_GRID_CONFIG } from './stock-adjustment-grid.config';
import { STOCK_ADJUSTMENT_TOOLBAR_CONFIG } from './stock-adjustment-toolbar.config';
...
export class StockAdjustmentListComponent implements OnInit, OnDestroy {
  ...
  readonly toolbarConfig = STOCK_ADJUSTMENT_TOOLBAR_CONFIG;
  readonly gridConfig = STOCK_ADJUSTMENT_GRID_CONFIG;

  readonly rows = signal<StockAdjustment[]>([]);
  ...
  onGridAction(event: GridActionEvent<StockAdjustment>): void {
    if (!event.row) {
      return;
    }
    if (event.action === 'edit') {
      this.router.navigate(['/inventory/stock-adjustments', event.row.id]);
    }
    if (event.action === 'delete') {
      this.stockAdjustmentService.delete(event.row.id, event.row.version).subscribe({
        next: () => this.load(),
      });
    }
  }

  onRowDoubleClick(event: GridRowClickEvent<StockAdjustment>): void {
    this.router.navigate(['/inventory/stock-adjustments', event.row.id]);
  }
