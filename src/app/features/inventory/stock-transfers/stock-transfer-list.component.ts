...
import { STOCK_TRANSFER_GRID_CONFIG } from './stock-transfer-grid.config';
import { STOCK_TRANSFER_TOOLBAR_CONFIG } from './stock-transfer-toolbar.config';
...
export class StockTransferListComponent implements OnInit, OnDestroy {
  ...
  readonly toolbarConfig = STOCK_TRANSFER_TOOLBAR_CONFIG;
  readonly gridConfig = STOCK_TRANSFER_GRID_CONFIG;

  readonly rows = signal<StockTransfer[]>([]);
  ...
  onGridAction(event: GridActionEvent<StockTransfer>): void {
    if (!event.row) {
      return;
    }
    if (event.action === 'edit') {
      this.router.navigate(['/inventory/stock-transfers', event.row.id]);
    }
    if (event.action === 'delete') {
      this.stockTransferService.delete(event.row.id, event.row.version).subscribe({
        next: () => this.load(),
      });
    }
  }

  onRowDoubleClick(event: GridRowClickEvent<StockTransfer>): void {
    this.router.navigate(['/inventory/stock-transfers', event.row.id]);
  }
