...
import { MEDICINE_GENERIC_GRID_CONFIG } from './medicine-generic-grid.config';
import { MEDICINE_GENERIC_TOOLBAR_CONFIG } from './medicine-generic-toolbar.config';
import { MedicineGeneric } from './medicine-generic.models';
import { MedicineGenericService } from './medicine-generic.service';

@Component({
  selector: 'app-medicine-generic-list',
  standalone: true,
  imports: [AppToolbarComponent, AppGridComponent],
  templateUrl: './medicine-generic-list.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class MedicineGenericListComponent implements OnInit, OnDestroy {
  private readonly genericService = inject(MedicineGenericService);
  private readonly router = inject(Router);
  private readonly shortcuts = inject(KeyboardShortcutService);

  readonly toolbarConfig = MEDICINE_GENERIC_TOOLBAR_CONFIG;
  readonly gridConfig = MEDICINE_GENERIC_GRID_CONFIG;

  readonly rows = signal<MedicineGeneric[]>([]);
  ...
  onGridAction(event: GridActionEvent<MedicineGeneric>): void {
    if (!event.row) {
      return;
    }
    if (event.action === 'edit') {
      this.router.navigate(['/medicine/generics', event.row.id]);
    }
    if (event.action === 'delete') {
      this.genericService.delete(event.row.id, event.row.version).subscribe({
        next: () => this.load(),
      });
    }
  }

  onRowDoubleClick(event: GridRowClickEvent<MedicineGeneric>): void {
    this.router.navigate(['/medicine/generics', event.row.id]);
  }

  private onNew(): void {
    this.router.navigate(['/medicine/generics/new']);
  }
