...
import { MEDICINE_CATEGORY_GRID_CONFIG } from './medicine-category-grid.config';
import { MEDICINE_CATEGORY_TOOLBAR_CONFIG } from './medicine-category-toolbar.config';
...
export class MedicineCategoryListComponent implements OnInit, OnDestroy {
  ...
  readonly toolbarConfig = MEDICINE_CATEGORY_TOOLBAR_CONFIG;
  readonly gridConfig = MEDICINE_CATEGORY_GRID_CONFIG;

  readonly rows = signal<MedicineCategory[]>([]);
  ...
  onGridAction(event: GridActionEvent<MedicineCategory>): void {
    if (!event.row) {
      return;
    }
    if (event.action === 'edit') {
      this.router.navigate(['/medicine/categories', event.row.id]);
    }
    if (event.action === 'delete') {
      this.categoryService.delete(event.row.id, event.row.version).subscribe({
        next: () => this.load(),
      });
    }
  }

  onRowDoubleClick(event: GridRowClickEvent<MedicineCategory>): void {
    this.router.navigate(['/medicine/categories', event.row.id]);
  }

  private onNew(): void {
    this.router.navigate(['/medicine/categories/new']);
  }
