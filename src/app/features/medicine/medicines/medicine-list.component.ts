...
import { MEDICINE_GRID_CONFIG } from './medicine-grid.config';
import { MEDICINE_TOOLBAR_CONFIG } from './medicine-toolbar.config';
...
export class MedicineListComponent implements OnInit, OnDestroy {
  ...
  readonly toolbarConfig = MEDICINE_TOOLBAR_CONFIG;
  readonly gridConfig = MEDICINE_GRID_CONFIG;
  ...
  onGridAction(event: GridActionEvent<Medicine>): void {
    if (!event.row) {
      return;
    }
    if (event.action === 'edit') {
      this.router.navigate(['/medicine/medicines', event.row.id]);
    }
    if (event.action === 'delete') {
      this.medicineService.delete(event.row.id, event.row.version).subscribe({
        next: () => this.load(),
      });
    }
  }

  onRowDoubleClick(event: GridRowClickEvent<Medicine>): void {
    this.router.navigate(['/medicine/medicines', event.row.id]);
  }

  private onNew(): void {
    this.router.navigate(['/medicine/medicines/new']);
  }
