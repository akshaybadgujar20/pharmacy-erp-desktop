...
import { LEDGER_GRID_CONFIG } from './ledger-grid.config';
import { LEDGER_TOOLBAR_CONFIG } from './ledger-toolbar.config';
...
export class LedgerListComponent implements OnInit, OnDestroy {
  ...
  readonly toolbarConfig = LEDGER_TOOLBAR_CONFIG;
  readonly gridConfig = LEDGER_GRID_CONFIG;

  readonly rows = signal<Ledger[]>([]);
  ...
  onGridAction(event: GridActionEvent<Ledger>): void {
    if (!event.row) {
      return;
    }
    if (event.action === 'edit') {
      this.router.navigate(['/finance/ledgers', event.row.id]);
    }
    if (event.action === 'delete') {
      this.ledgerService.delete(event.row.id, event.row.version).subscribe({
        next: () => this.load(),
      });
    }
  }

  onRowDoubleClick(event: GridRowClickEvent<Ledger>): void {
    this.router.navigate(['/finance/ledgers', event.row.id]);
  }

  private onNew(): void {
    this.router.navigate(['/finance/ledgers/new']);
  }
