...
import { AREA_GRID_CONFIG } from './area-grid.config';
import { AREA_TOOLBAR_CONFIG } from './area-toolbar.config';
...
export class AreaListComponent implements OnInit, OnDestroy {
  ...
  readonly toolbarConfig = AREA_TOOLBAR_CONFIG;
  readonly gridConfig = AREA_GRID_CONFIG;

  readonly rows = signal<Area[]>([]);
  ...
  onGridAction(event: GridActionEvent<Area>): void {
    if (!event.row) {
      return;
    }
    if (event.action === 'edit') {
      this.router.navigate(['/masters/areas', event.row.id]);
    }
    if (event.action === 'delete') {
      this.areaService.delete(event.row.id, event.row.version).subscribe({
        next: () => this.load(),
      });
    }
  }

  onRowDoubleClick(event: GridRowClickEvent<Area>): void {
    this.router.navigate(['/masters/areas', event.row.id]);
  }

  private onNew(): void {
    this.router.navigate(['/masters/areas/new']);
  }
