...
import { SALES_INVOICE_GRID_CONFIG } from './sales-invoice-grid.config';
import { SALES_INVOICE_TOOLBAR_CONFIG } from './sales-invoice-toolbar.config';
...
export class SalesInvoiceListComponent implements OnInit, OnDestroy {
  ...
  readonly toolbarConfig = SALES_INVOICE_TOOLBAR_CONFIG;
  readonly gridConfig = SALES_INVOICE_GRID_CONFIG;

  readonly rows = signal<SalesInvoice[]>([]);
  ...
  onGridAction(event: GridActionEvent<SalesInvoice>): void {
    if (!event.row) {
      return;
    }
    if (event.action === 'edit') {
      this.router.navigate(['/sales/invoices', event.row.id]);
    }
    if (event.action === 'delete') {
      this.salesInvoiceService.delete(event.row.id, event.row.version).subscribe({
        next: () => this.load(),
      });
    }
  }

  onRowDoubleClick(event: GridRowClickEvent<SalesInvoice>): void {
    this.router.navigate(['/sales/invoices', event.row.id]);
  }
