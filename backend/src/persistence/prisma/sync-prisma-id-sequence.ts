...
export async function syncPrismaIdSequenceFromDatabase(
  ...
  await syncBigIntIdSequenceFromDb(async () => {
    const ids = await Promise.all([
      ...
      maxId(client, () =>
        client.salesInvoice.findFirst({
          orderBy: { id: 'desc' },
          select: { id: true },
        }),
      ),
      maxId(client, () =>
        client.outbox.findFirst({
          orderBy: { id: 'desc' },
          select: { id: true },
        }),
      ),
      maxId(client, () =>
        client.stock.findFirst({
          orderBy: { id: 'desc' },
          select: { id: true },
        }),
      ),
      maxId(client, () =>
        client.batch.findFirst({
          orderBy: { id: 'desc' },
          select: { id: true },
        }),
      ),
      maxId(client, () =>
        client.medicine.findFirst({
          orderBy: { id: 'desc' },
          select: { id: true },
        }),
      ),
      ...
      maxId(client, () =>
        client.sequenceGenerator.findFirst({
          orderBy: { id: 'desc' },
          select: { id: true },
        }),
      ),
